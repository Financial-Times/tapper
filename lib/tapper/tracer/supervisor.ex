defmodule Tapper.TracerSupervisor do
    ...
    def init([config]) do
        Logger.debug("TracerSupervisor init #{:erlang.pid_to_list(self())}")
        supervise(
            [worker(Tapper.Tracer, [config], restart: :temporary)], # template for children, with config as first param

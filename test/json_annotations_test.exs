defmodule JsonAnnotationsTest do
  ...
  test "encode binary annotation value" do
    ...
    assert Tapper.Encoder.Json.encode_binary_annotation_value(:i16, 32767) == 32767
    assert_raise ArgumentError, fn -> Tapper.Encoder.Json.encode_binary_annotation_value(:i16, 32768) end

    assert Tapper.Encoder.Json.encode_binary_annotation_value(:i32, 4294967295) == 4294967295
    assert_raise ArgumentError, fn -> Tapper.Encoder.Json.encode_binary_annotation_value(:i32, 4294967296) end

    assert Tapper.Encoder.Json.encode_binary_annotation_value(:i64, 9007199254740991) == 9007199254740991
    assert Tapper.Encoder.Json.encode_binary_annotation_value(:i64, 9007199254740992) == "9007199254740992"
    assert_raise ArgumentError, fn -> Tapper.Encoder.Json.encode_binary_annotation_value(:i64, 1.8446744073709552e19) end

    assert Tapper.Encoder.Json.encode_binary_annotation_value(:bytes, <<1,2,3,4>>) == Base.encode64(<<1,2,3,4>>)
  end
  ...
  test "encode binary annotation" do
    ...
    as_string = Tapper.Encoder.Json.encode_binary_annotation(%Tapper.Protocol.BinaryAnnotation{annotation_type: :string, key: "key_for_string", value: "string-value", host: host})
    assert as_string == %{key: "key_for_string", value: "string-value", endpoint: endpoint}

    as_i16 = Tapper.Encoder.Json.encode_binary_annotation(%Tapper.Protocol.BinaryAnnotation{annotation_type: :i16, key: "key_for_i16", value: 1024, host: host})
